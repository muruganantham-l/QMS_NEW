...
@model			varchar(100) ='DCI 4511',
@batch			varchar(100) ='BATCH 3',
@supp_name		varchar(100)='SKYDENTAL MALAYSIA SDN BHD' ,
@manufacture	varchar(100)	='Dental Components LLC'	
as 
...
WHERE (wkr_mst.site_cd = wkr_det.site_cd)
	...
	AND (wkr_mst.wkr_mst_wr_status <> 'D')
	AND (ast_mst.site_cd = 'QMS')
	--commented by murugan AND ast_mst.ast_mst_asset_code = isnull(@reporttype,ast_mst.ast_mst_asset_code)
	--commented by murugan AND ast_det.ast_det_varchar15 in (select Ownership_Type from ownership_mst (nolock) where Ownership_desc like @ownership )
	AND wkr_mst.wkr_mst_org_date between  @startdate_temp and @enddate_temp
	
and ast_det_varchar21			= @batch
and ast_det_varchar16			= @supp_name
and ast_det_mfg_cd				= @manufacture
and ast_mst_asset_longdesc		= @be_category
and  ast_det_modelno			=  @model
...
Delete from Tsd_penalty_report_tab_tmp
where [Guid] = @guid
*/
set nocount off
