Alter view Zone_Scorecard_view
as
select 
ast_mst_perm_id 'State Name', '1.WO Received' Types ,Count(wko_mst_wo_no) 'NumberOf WO'
from wko_mst (nolock) 
,wko_det (nolock)
,wkr_mst (nolock)
,ast_mst (nolock)
...
select 
ast_mst_perm_id 'State Name','2.WO Completed' Types, Count(wko_mst_wo_no) 'NumberOf WO'
from wko_mst (nolock) 
,wko_det (nolock)
,wkr_mst (nolock)
,ast_mst (nolock)
...
select 
ast_mst_perm_id 'State Name','3.WO Pending' Types, Count(wko_mst_wo_no) 'NumberOf WO'
from wko_mst (nolock) 
,wko_det (nolock)
,wkr_mst (nolock)
,ast_mst (nolock)
