...
Declare @startdate varchar(10)  = '20170101'
Declare @enddate varchar(10) = '20170930'
...
Truncate Table PENALTY_REPORT_DATA_OUT
...
select @enddate_temp = CONVERT(VARCHAR(10),DATEADD(SS,-1,Dateadd(mm,1,@startdate_temp)),112)
...
SELECT Difference = COALESCE(DateDiff(minute, wko_mst.wko_mst_org_date, wko_det.wko_det_cmpl_date), 0)
	,CONVERT(DATETIME, convert(CHAR(8), @startdate_temp)) AS reportmonth
	...
	,CASE 
		WHEN ast_det_asset_cost >= 1
			AND ast_det_asset_cost <= 5000
			THEN 10
		WHEN ast_det_asset_cost >= 5001
			AND ast_det_asset_cost <= 10000
			THEN 20
		WHEN ast_det_asset_cost >= 10001
			AND ast_det_asset_cost <= 15000
			THEN 30
		WHEN ast_det_asset_cost >= 15001
			AND ast_det_asset_cost <= 20000
			THEN 40
		WHEN ast_det_asset_cost >= 20001
			AND ast_det_asset_cost <= 25000
			THEN 50
		WHEN ast_det_asset_cost >= 25001
			AND ast_det_asset_cost <= 30000
			THEN 60
		WHEN ast_det_asset_cost >= 30001
			AND ast_det_asset_cost <= 35000
			THEN 70
		WHEN ast_det_asset_cost >= 35001
			AND ast_det_asset_cost <= 40000
			THEN 80
		WHEN ast_det_asset_cost >= 40001
			AND ast_det_asset_cost <= 45000
			THEN 90
		WHEN ast_det_asset_cost >= 45001
			AND ast_det_asset_cost <= 50000
			THEN 100
		WHEN ast_det_asset_cost >= 50001
			THEN 100
		END AS Penalty_Cost
	,CASE 
...
WHERE (wkr_mst.site_cd = wkr_det.site_cd)
	...
	AND Month(wkr_mst.wkr_mst_org_date) = month(@startdate_temp)
	AND Year(wkr_mst.wkr_mst_org_date) = year(@startdate_temp)
...
UNION ALL
	...
	,Response_Actual_KPI = CASE 
		...
		WHEN DATEDIFF(MI, wkr_mst.wkr_mst_org_date, DATEADD(s, - 1, DATEADD(mm, DATEDIFF(m, 0, wkr_mst.wkr_mst_org_date) + 1, 0))) / 1440 - (2 * DATEDIFF(wk, wkr_mst.wkr_mst_org_date, DATEADD(s, - 1, DATEADD(mm, DATEDIFF(m, 0, wkr_mst.wkr_mst_org_date) + 1, 0)))) > 2
			THEN 0
		END
	,Repair_Actual_KPI = CEILING( CAST(DateDiff(minute, CONVERT(DATETIME, convert(CHAR(8), @startdate_temp)), CASE 
					WHEN wko_det.wko_det_cmpl_date IS NOT NULL
						AND wko_det.wko_det_cmpl_date < CONVERT(DATETIME, convert(CHAR(8), @enddate_temp))
						THEN wko_det.wko_det_cmpl_date
					ELSE DATEADD(s, - 1, DATEADD(mm, DATEDIFF(m, 0, CONVERT(DATETIME, convert(CHAR(8), @startdate_temp))) + 1, 0))
					END) AS DECIMAL(14, 5)) / 60 / 24)
	, Repair_Actual_KPI_curr = CEILING(CAST(DateDiff(minute, wkr_mst.wkr_mst_org_date, CASE 
					WHEN wko_det.wko_det_cmpl_date < DATEADD(s, - 1, DATEADD(mm, DATEDIFF(m, 0, wkr_mst.wkr_mst_org_date) + 1, 0))
						AND wko_det_cmpl_date IS NOT NULL
...
WHERE (wkr_mst.site_cd = wkr_det.site_cd)
	...
	SELECT @startdate_temp = cONVERT(VARCHAR(10),DATEADD(DD,1,CONVERT(DATETIME,@enddate_temp)),112)
	 

END

Tsd_penalty_report_tab
where [WO Number] in ('CWO157614','CWO157615')
