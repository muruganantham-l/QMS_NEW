...
,[Repair Computation Field KPI]
,[Repair Holidays&Weekends]
,year([Wr Datetime]) 'WR Year'
--,convert(varchar,month([Wr Datetime]))+'.'+Left(Datename(mm,[Wr Datetime]),3) 'wr_month_name'
,Left(Datename(mm,[Wr Datetime]),3) 'wr_month_name'
,month([Wr Datetime]) 'wr_month_num'
,97 'strip_lines'
 ...
 --SELECT convert(varchar,month([Wr Datetime]))+'.'+Left(Datename(mm,[Wr Datetime]),3) 'wr_month_name'
 --from tsd_performance_kpi_tab (nolock)  
set nocount off
