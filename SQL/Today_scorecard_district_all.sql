alter proc  Today_scorecard_district_all
@state_code varchar(100) = 'SBH'
...
--all
SELECT s.district_code 'State Name',equipment_type 'Equip.Type',year_wo 'Year OF WO',month_wo 'Month OF WO','1.WO Received' Types ,Count(wo_no) 'NumberOf WO'
from score_card_tbl (NOLOCK) s
where wo_type = 'cwo'
and wkr_mst_org_date = cast(getdate() as date)
and statename = @state_code
and s.equipment_type = @eqip_type
GROUP by s.district_code,equipment_type,year_wo,month_wo
--all pending 
union ALL
SELECT s.district_code,equipment_type,year_wo,month_wo,'2.WO Pending'   Types ,Count(wo_no) 'NumberOf WO'
from score_card_tbl (NOLOCK) s
where wo_type = 'cwo'
...
union ALL
SELECT s.district_code,equipment_type,year_wo,month_wo,'3.WO Pending without MR'   Types ,Count(wo_no) 'NumberOf WO'
from score_card_tbl (NOLOCK) s
where wo_type = 'cwo'
and  wko_mst_status in ('OPE','RFS')
and wkr_mst_org_date = cast(getdate() as date)
