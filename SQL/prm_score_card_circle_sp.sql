alter proc prm_score_card_circle_sp
@state_code varchar(100) = 'bpg'
...
--SELECT * INTO PRM_score_card_circle_tbl FROM PRM_score_card_circle_tbl
declare @year int = year(getdate())-- @year-- 
truncate table PRM_Scorecard_view_All_circle_temp
truncate table PRM_score_card_circle_tbl
--alter table PRM_score_card_circle_tbl
--add   statename varchar(50)
insert PRM_score_card_circle_tbl
(
circlename--
,ast_mst_asset_no--
,equipment_type -- no
...
,wko_mst_ast_cod
,statename
from score_card_PRM_tbl (NOLOCK)
...
update  PRM_score_card_circle_tbl
...
from  PRM_score_card_circle_tbl a join ast_mst m on a.ast_mst_parent_id = m.ast_mst_asset_no and a.equipment_type = 'NA'
join ast_det d on m.RowID = d.mst_RowID
 
 */
--update a
--set		mr_no	=	m.mtr_mst_mtr_no
--from PRM_score_card_circle_tbl a join mtr_mst m on m.mtr_mst_wo_no = a.wo_no
--alter table PRM_Scorecard_view_All_circle_temp
--add state_code varchar(50)

insert PRM_Scorecard_view_All_circle_temp
(
[circle Name]
,[Equip.Type]
,[Year OF WO]
...
--all
SELECT s.circlename,equipment_type,year_wo,month_wo,'1.PWO Generated' Types ,Count(wo_no) 'NumberOf WO',max(statename)
from PRM_score_card_circle_tbl (NOLOCK) s
where wo_type = 'PWO'
and equipment_type = @eqip_type
GROUP by s.circlename,equipment_type,year_wo,month_wo
--all pending 
union ALL
SELECT s.circlename,equipment_type,year_wo,month_wo,'2.PWO Pending'   Types ,Count(wo_no) 'NumberOf WO',max(statename)
from PRM_score_card_circle_tbl (NOLOCK) s
where wo_type = 'PWO'
and equipment_type = @eqip_type
...
--pending without mr
--union ALL
--SELECT s.circlename,equipment_type,year_wo,month_wo,'3.WO Pending without MR'   Types ,Count(wo_no) 'NumberOf WO'
--from PRM_score_card_circle_tbl (NOLOCK) s
--where wo_type = 'cwo'
--and  wko_mst_status in ('OPE','RFS')
--and mr_no is null
...
--delete v1 from PRM_Scorecard_view_All_circle_temp v1 where
--not EXISTS (SELECT '' from PRM_Scorecard_view_All_circle_temp v2 (nolock) where v1.[Year OF WO] = v2.[Year OF WO] and v1.[Month OF WO] = v2.[Month OF WO]
...
delete v1 from PRM_Scorecard_view_All_circle_temp v1 where
not EXISTS (SELECT '' from PRM_Scorecard_view_All_circle_temp v2 (nolock) where v1.[Year OF WO] = v2.[Year OF WO] and v1.[Month OF WO] = v2.[Month OF WO]
and v1.state_code = v2.state_code and v1.[Equip.Type] = v2.[Equip.Type]   and v2.Types='2.PWO Pending' and v2.[NumberOf WO] > 0)
...
,[Types]
,sum([NumberOf WO])
from PRM_Scorecard_view_All_circle_temp (NOLOCK)
group by [Equip.Type]
,[Year OF WO]
,[Month OF WO]
,[Types]
...
,[NumberOf WO]
 ...
 from PRM_Scorecard_view_All_circle_temp (NOLOCK) t 
 --join circle_desc_qms d on t.[circle Name] =d.circle_desc
 
 --alter table PRM_Scorecard_view_All_circle_temp alter column types varchar(20)
