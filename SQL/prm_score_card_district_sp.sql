...
--alter table PRM_Scorecard_view_All_district_temp  add  state_code varchar(30)
...
--all
SELECT s.districtname,equipment_type,year_wo,month_wo,'1.PWO Generated' Types ,Count(wo_no) 'NumberOf WO',max(state_code)
from PRM_score_card_district_tbl (NOLOCK) s
where wo_type = 'PWO'
--and s.equipment_type = @equip_type
GROUP by s.districtname,equipment_type,year_wo,month_wo
--all pending 
union ALL
SELECT s.districtname,equipment_type,year_wo,month_wo,'2.PWO Pending'   Types ,Count(wo_no) 'NumberOf WO',max(state_code)
from PRM_score_card_district_tbl (NOLOCK) s
where wo_type = 'PWO'
...
,state_code
 ...
 from PRM_Scorecard_view_All_district_temp (NOLOCK) t 
