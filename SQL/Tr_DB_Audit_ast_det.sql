...
/****** Object:  Trigger [dbo].[Tr_DB_Audit_ast_det]    Script Date: 1/10/2018 10:38:18 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[Tr_DB_Audit_ast_det] ON [dbo].[ast_det] FOR INSERT, UPDATE, DELETE
AS
...
BEGIN
	   ...
	   --select @field '@field',SUBSTRING(COLUMNS_UPDATED(),@char, 1),@bit '@bit', @Type '@Type',@char '@char'
               SELECT @fieldname = COLUMN_NAME 
                       FROM INFORMATION_SCHEMA.COLUMNS 
                       WHERE TABLE_NAME = @TableName 
                       AND ORDINAL_POSITION = case @field when 114 then 113 else @field end
...
select ''' + @Type + ''',''' 
       ...
       + ' or (i.' + @fieldname + ' is not null and  d.' 
               ...
               EXEC (@sql)
			     ...
			     if @fieldname in ('ast_det_cus_code','ast_det_numeric8','ast_det_numeric9')
			   begin
			   ...
			   select @fieldname1  = case   @fieldname1 when 'ast_det_cus_code' then 'Clinic Code' when 'ast_det_numeric8' then 'Monthly Rev' when 'ast_det_numeric9' then 'Rental Per Month'  else null end
...
select ''' + @Type + ''',''' 
  ...
  if (select count(*) from  Audit1 where  sessiod_id = @guid  ) > 0 
   begin
			   ...
			   ,'',case   S.FieldName   when 'ast_det_cus_code' then 'Clinic Code' when 'ast_det_numeric8' then 'Monthly Rev' when 'ast_det_numeric9' then 'Rental Per Month'  else null end AS 'td'
			   ,'',isnull(cast(s.OldValue as varchar(300)),'') AS 'td'
			   ,'',isnull(cast(S.newvalue as varchar(300)),'') AS 'td'
			   ,'',S.updatedate AS 'td'
			   ,'',S.username AS 'td'
			  -- ,'', SUSER_NAME() AS 'td'
        FROM	Audit1 s
