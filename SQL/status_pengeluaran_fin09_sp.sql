...
ALTER proc status_pengeluaran_fin09_sp
...
select @uptime_guarantees		= ltrim(rtrim(@uptime_guarantees))
		...
		if exists (select '' from status_pengeluaran_fin09_tbl (nolock) where state1 = @state and year1 = @year)
		begin

			update status_pengeluaran_fin09_tbl
			set 
		...
		begin
			insert status_pengeluaran_fin09_tbl
			(
			state1
			,year1
			,response_time
